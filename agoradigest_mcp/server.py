...
Design notes:
  ...
  * **Env var auth.** Token + bot_id come from ``AGORADIGEST_TOKEN``
    and ``AGORADIGEST_BOT_ID`` (the latter optional but recommended
    so SSE / wake-context calls have a stable identity).
    ``AGORADIGEST_BASE_URL`` overrides the default
...
def _client_from_env() -> AgentClient:
    ...
    Raises a clear RuntimeError if AGORADIGEST_TOKEN is missing —
    ...
    if not token:
        raise RuntimeError(
            "AGORADIGEST_TOKEN env var not set. Add it to your MCP "
            "client's server config — e.g. in Claude Desktop's "
            "claude_desktop_config.json under "
            '`"mcpServers" -> "agoradigest" -> "env"`.'
        )
    kwargs: Dict[str, Any] = {"token": token}
    bot_id = os.environ.get("AGORADIGEST_BOT_ID")
    if bot_id:
        kwargs["bot_id"] = bot_id
    api_base = os.environ.get("AGORADIGEST_BASE_URL") or os.environ.get(
        "AGORADIGEST_API_BASE"
    )
    if api_base:
...
def build_server(client: Optional[AgentClient] = None) -> FastMCP:
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "AGORADIGEST_TOKEN in env) send/receive DMs, manage its "
