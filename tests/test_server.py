...
from agoradigest_mcp.server import (
    _client_from_env,
    _envelope_to_dict,
    _list_of_dicts,
    build_server,
...
def test_client_from_env_raises_without_token():
    with patch.dict(os.environ, {}, clear=True):
        with pytest.raises(RuntimeError, match="AGORADIGEST_TOKEN"):
            _client_from_env()


def test_client_from_env_with_just_token():
    with patch.dict(os.environ, {"AGORADIGEST_TOKEN": "bt_xxx"}, clear=True):
...
def test_client_from_env_passes_base_url():
    env = {
        "AGORADIGEST_TOKEN": "bt_xxx",
        "AGORADIGEST_BASE_URL": "https://api.staging.agoradigest.com",
    }
    with patch.dict(os.environ, env, clear=True):
        client = _client_from_env()
...
def test_envelope_to_dict_dataclasses_friend():
    from agoradigest import Friend
...
def test_send_dm_calls_sdk():
    from agoradigest import TaskEnvelope
    client = MagicMock()
...
def test_context_for_wake_unpacks_all_fields():
    ...
    from agoradigest import WakeContext
    client = MagicMock()
...
def test_late_binding_client_boots_without_env():
    ...
    server boot must NOT require AGORADIGEST_TOKEN at module-load
...
def test_late_binding_first_tool_call_raises_with_helpful_msg():
    ...
    surface the AGORADIGEST_TOKEN error so the user knows to fix
    their MCP config."""
    with patch.dict(os.environ, {}, clear=True):
        mcp = build_server(client=None)
